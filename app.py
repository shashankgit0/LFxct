...
# ─── Hide Streamlit branding & force dark ──────────────────────────────────────
st.markdown("""
<style>
...
ADMIN_ROLES  = ["admin"]          # excluded from leaderboard/stats
...
def page_leaderboard():
    ...
    for u in users:
        ...
        rows.append({"Rank": "", "Player": u["display_name"],
                     "SP Pts": sp, "BP Pts": bp, "Streak Pts": streak, "Total": total})
...
def page_submit_sp():
    ...
    with col2:
        predicted_winner_raw = st.text_input("Predicted Winner (team name)")
...
def page_bp_approvals():
    ...
    for bp in pending:
        ...
        # Queen approves only Pawns' BPs (not own, not Bishops, not Knight)
        if role == "bp_manager":
        ...
        shown += 1
        # Queen & King see name; Knight sees hidden
...
def page_panel_scoring():
    ...
    # Knight rates Bishops' BPs (blind)
    if role == "knight":
        to_rate = []
        for bp in approved:
    ...
    # Bishops rate Pawns' BPs (blind, not own)
    ...
    if role == "admin":
        ...
        for bp in unrated:
            ...
            score = st.radio("Rating (P1 slot):", [0,1,2,3], horizontal=True, key=f"admin_{bp['id']}")
...
def page_enter_results():
    ...
    actual_winner_raw = st.text_input("Actual Winner (team name)")
...
def page_match_details():
    ...
    if bps:
        ...
        for bp in bps:
            ...
            bp_rows.append({
                "": icon, "Player": get_user_display(bp["player"]),
                "Prediction": bp["prediction"],
                "P1": bp.get("panel1_score","-"), "P2": bp.get("panel2_score","-"), "P3": bp.get("panel3_score","-"),
                "Avg": bp.get("avg_score","-"), "Pts": bp.get("points_awarded",0)
            })
    ...
    if preds:
        ...
        for i, p in enumerate(preds_sorted):
            ...
            exact = "⚡ Exact!" if m.get("actual_score") and int(p.get("predicted_score") or 0) == m.get("actual_score") else ""
            sp_rows.append({
                "": rank, "Player": get_user_display(p["player"]),
                "Predicted Score": f"{p.get('predicted_score')} - {str(p.get('predicted_wickets',0)).zfill(2)}",
                "Predicted Winner": p.get("predicted_winner","-"),
                "Actual Score": f"{m.get('actual_score','-')} - {str(m.get('actual_wickets',0)).zfill(2)}" if m.get("actual_score") else "-",
                "Exact?": exact, "Pts": p.get("points_awarded",0)
            })
    ...
    if selected == "— Overall —":
        ...
        if bps:
            st.dataframe(pd.DataFrame([{
                "Match": b["match_name"], "Prediction": b["prediction"],
                "Avg Panel": b.get("avg_score","Pending"),
                "Result": b.get("result","pending"), "Pts": b.get("points_awarded",0)
...
""")
    ...
    st.markdown("""
- Submit once **before** the tournament starts
- 🧡 Orange Cap → **20 pts**
- 💜 Purple Cap → **20 pts**
- 🌟 Emerging Player → **15 pts**
- 🏏 Correct Top 4 team → **6 pts each** (+4 if position also correct)
""")
    st.markdown("---")
    st.subheader("♟ Roles")
    st.markdown("""
...
| ♕ | Queen | Approves Pawn BPs, enters results (rotates weekly) |
| ♞ | Knight | Approves Queen's + Bishops' BPs blindly, rates Bishops' BPs blindly (rotates weekly) |
| ♗ | Bishop x3 | Rates Pawn BPs blindly 0–3 (fixed all tournament) |
| ♟ | Pawn | Submits BPs and SPs, plays the game |
""")
    ...
    with tab2:
        ...
        if st.button("Add Player"):
            if nu.strip() and np.strip() and nd.strip():
                ...
                st.success(f"✅ {nd} added as {role_labels_map[nr]}!")
                st.rerun()
        st.markdown("---")
